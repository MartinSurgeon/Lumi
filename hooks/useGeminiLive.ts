...
import { StudentProfile, ConnectionStatus, Message, ImageResolution, LearningStats, Session } from '../types';
...
export const useGeminiLive = ({ profile, videoRef, imageResolution }: UseGeminiLiveProps) => {
    ...
    // Refs for tracking session data without staleness
    ...
    useEffect(() => {
        ...
        return () => {
            // Cleanup on unmount handled by disconnect via effect cleanup if needed
        };
    ...
    const disconnect = useCallback(async () => {
        ...
        if (isLiveRef.current && startTimeRef.current && profile) {
            const duration = Math.round((Date.now() - startTimeRef.current) / 1000);

            // Only save meaningful sessions (> 10 seconds)
            if (duration > 10) {
                const session: Session = {
                    id: Date.now().toString(),
                    date: new Date().toISOString(),
                    duration,
                    subject: profile.favoriteSubject, // Fallback to favorite, ideally would track dynamic topic
                    topic: profile.struggleTopic,
                    score: learningStatsRef.current.understandingScore
                };

                try {
                    const existing = localStorage.getItem('lumi_sessions');
                    const sessions = existing ? JSON.parse(existing) : [];
                    sessions.unshift(session); // Add to top
                    localStorage.setItem('lumi_sessions', JSON.stringify(sessions));
    ...
    }, [profile]); // profile is needed for saving session
    ...
    const connect = useCallback(async () => {
        ...
        if (!hasEnvKey && !hasAiStudio) {
            alert("API Key is missing! Check GEMINI_API_KEY in your .env file.");
        ...
        if (!audioContextRef.current || audioContextRef.current.state === 'closed') {
            ...
            audioContextRef.current.addEventListener('statechange', () => {
                ...
                if (ctx) {
                    ...
                    // Auto-resume if suspended during active session
                    if (ctx.state === 'suspended' && isLiveRef.current) {
                        ctx.resume().then(() => {
                            console.log('AudioContext auto-resumed after suspension');
                        }).catch(err => {
        ...
        const establishConnection = async () => {
            try {
                ...
                // CRITICAL CHECK: AudioContext might be null if disconnected during setup
                if (!audioContextRef.current) {
                    console.error("AudioContext is null during setup");
                    setStatus(ConnectionStatus.ERROR);
                    return;
                }
                const ctx = audioContextRef.current;
                ...
                try {
                    ...
                    setMessages(prev => [...prev, {
                        id: Date.now().toString(),
                        role: 'system',
                        text: '⚠️ Microphone access denied. Please allow permissions to use voice.',
                        timestamp: new Date()
                    }]);
                    setStatus(ConnectionStatus.ERROR);
                    return;
                ...
                // CRITICAL CHECK: Ensure we are still connecting after await
                if (!audioContextRef.current) {
                    // Clean up stream if context died
                ...
                const config = {
                    ...
                    callbacks: {
                        onopen: async () => {
                            ...
                            // Mark start time for session analytics
                            ...
                            if (!currentCtx || currentCtx.state === 'closed') {
                                console.warn("AudioContext is null or closed during setup");
                                return;
                            }
