...
import { Mic, MicOff, Video, VideoOff, PhoneOff, Play, Sparkles, User, Bot, Zap, ImageIcon, Settings, Monitor, Search, BrainCircuit, TrendingUp, Award, BookOpen, Keyboard, Send, Clock, AlertCircle, ScanEye, Copy, Check, Heart, Flag, Ghost, RefreshCw, LogOut, HelpCircle, Volume2, MessageSquare, Radio, Download, Activity, TrendingDown, BarChart2 } from 'lucide-react';
      ...
      <header className="w-full px-4 py-3 flex justify-between items-center z-50 absolute top-0 left-0 pointer-events-none">
        ...
        <div className="flex items-center gap-3 pointer-events-auto">
           ...
           <button 
                ...
                className="p-2 text-fuchsia-400 hover:text-fuchsia-300 bg-black/20 rounded-full backdrop-blur-md border border-white/5 hover:bg-fuchsia-500/10 transition-all"
            ...
            <div className={`absolute left-1/2 -translate-x-1/2 z-20 flex gap-2 transition-all duration-700 ${isVideoActive ? 'top-4' : 'top-16 lg:top-8'}`}>
              ...
              {isTeacherMode && isConnected && (
                <div className="flex items-center gap-2 bg-amber-900/60 backdrop-blur-md border border-amber-500/30 px-3 py-1 rounded-full shadow-lg">
                    ...
                    <span className="text-[10px] font-bold text-amber-400 uppercase tracking-wide">Teacher Mode</span>
                </div>
              )}
            </div>
                            ...
                            if (msg.role === 'system') {
                                return (
                                    <div key={msg.id} className="flex justify-center w-full animate-fade-in my-4 px-4">
                                        <div className="w-full max-w-md bg-black/40 border border-white/10 rounded-lg p-3 shadow-lg backdrop-blur-md relative overflow-hidden group hover:bg-black/50 transition-colors">
                                            <div className="flex items-start gap-3 relative z-10">
                                                <div className="mt-0.5 p-1 bg-slate-800/50 rounded border border-white/5">
                                                    <Monitor size={12} className="text-cyan-400/80" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span className="text-[9px] font-bold text-cyan-500/50 uppercase tracking-widest font-mono">SYSTEM_LOG</span>
